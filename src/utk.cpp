    ...
    else
    {
        ...
        for (int ci = 0; ci < leaves.size(); ci++)
        {
            if (leaves[ci]->rank == k - 1)
            {
                ...
                if (finalRetOptions.find(focal%objCnt) == finalRetOptions.end())
                {
                    finalRetOptions.insert(focal%objCnt);
                }
                for (auto pIter = leaves[ci]->BelowHP.begin(); pIter != leaves[ci]->BelowHP.end(); pIter++)
                {
                    if (finalRetOptions.find(*pIter%objCnt) == finalRetOptions.end())
                    {
                        finalRetOptions.insert(*pIter%objCnt);
                    }
                }
                for (auto pIter = leaves[ci]->IntersectHP.begin(); pIter != leaves[ci]->IntersectHP.end(); pIter++)
                {
                    if (pIter->second == 1)
                    {
                        if (finalRetOptions.find(pIter->first%objCnt) == finalRetOptions.end())
                        {
                            finalRetOptions.insert(pIter->first%objCnt);
                        }
                    }
                }
            }
...
void UTK::jaa(vector<float>& region, vector<cell*>& exactutk, int& k, const int dim, float* PG[], Rtree& a_rtree)
{
    ...
    {
        cout<< "option size: "<<finalRetOptions.size()<<endl;
        cout << epoch << ", result size: " << exactutk.size() << "; remain subregions: " << subregions.size() << endl;
        epoch++;
        subreg = subregions.front();
        subregions.pop();
        if (epoch*objCnt+objCnt > 2 >> 31) continue; // Jiahao: bugs when too much epoches
